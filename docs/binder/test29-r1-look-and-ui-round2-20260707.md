...
| Random zone created a Temperate world with no roll log | Log evidence: `randomZone` was false at the Create click (single screen instance; wiring verified correct in code; only writer is a row click) — cannot distinguish "re-clicked a band" from a state bug after the fact | `beginExpedition` now ALWAYS logs `random=… selectedZone=…` at the click, so any recurrence is diagnosable in one line. Watch the row highlight before clicking Create |
