...
public final class LatitudeWorldLauncher {
    ...
    public static void beginExpedition(MinecraftClient client, LatitudeCreateWorldScreen screen,
        ...
        LOGGER.info("[Latitude lifecycle] begin expedition — type={}, size={}, zone={}, {}ms since beginExpedition",
                ...
                size.name(), spawnZone.id(), elapsedSinceLatitudeLoadingMs());
        try {
            ...
            RegistryEntry<WorldPreset> presetEntry = presetRegistry.getEntry(presetKey).orElse(null);
            if (presetEntry == null) {
            ...
            Registry<WorldPreset> updatedPresetRegistry = goh.getCombinedRegistryManager()
                    .get(RegistryKeys.WORLD_PRESET);
            RegistryEntry<WorldPreset> updatedPresetEntry = updatedPresetRegistry.getEntry(presetKey).orElse(null);
    ...
    private static void beginLatitudeLoading(long startedAtMs) {
        LatitudeClientState.firstWorldLoadStartMs = startedAtMs;
    }

    private static void activateLatitudeLoading() {
        if (LatitudeClientState.firstWorldLoadStartMs == 0L) {
            LatitudeClientState.firstWorldLoadStartMs = System.currentTimeMillis();
        }
    }

    private static long elapsedSinceLatitudeLoadingMs() {
        long startedAtMs = LatitudeClientState.firstWorldLoadStartMs;
        return startedAtMs > 0L ? System.currentTimeMillis() - startedAtMs : 0L;
    }

    private static void clearLatitudeLoadingState() {
