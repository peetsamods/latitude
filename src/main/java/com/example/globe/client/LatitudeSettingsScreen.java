...
public class LatitudeSettingsScreen extends Screen {
    ...
    protected void init() {
        ...
        if (compassExpanded) {
            baseY = y;
            var wCompassStyle = this.addDrawableChild(CyclingButtonWidget.<CompassHudConfig.CompassStyle>builder(v -> Text.literal(v == CompassHudConfig.CompassStyle.ANALOG ? "Analog" : "Digital"))
            ...
            baseY = y;
            var wCompassCompact = this.addDrawableChild(CyclingButtonWidget.<Boolean>builder(v -> Text.literal(v ? "ON" : "OFF"))
                    .values(true, false)
        ...
        baseY = y;
        var wDisplayZone = this.addDrawableChild(CyclingButtonWidget.<Boolean>builder(v -> Text.literal(v ? "ON" : "OFF"))
                .values(true, false)
    ...
    private void drawScaledCenteredText(DrawContext context, Text text, int centerX, int y, float scale, int color, boolean shadow) {
        ...
        matrices.push();
        matrices.translate((float) centerX, (float) y, 0.0f);
