...
public final class PoleArrivalPrewarm {
    ...
    /**
     * Band membership: inside the pole PROMPT band plus {@link #BAND_MARGIN_BLOCKS} --
     * {@code distToPole <= promptAt + margin}. NaN distance (bad read) is never in-band, so a transient bad
     * read drops the ticket rather than pinning one forever.
     */
    public static boolean inPrewarmBand(double distToPole, double promptAt) {
        if (Double.isNaN(distToPole) || Double.isNaN(promptAt)) {
            return false;
        }
        return distToPole <= promptAt + BAND_MARGIN_BLOCKS;
    }
